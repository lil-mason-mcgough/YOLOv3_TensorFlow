...
def _queue_evaluate(args, queue):
    queue.put(evaluate(args))
...
if __name__ == '__main__':
    ...
    parser.add_argument("--get_roc", action='store_true',
                        help="If set, adjusts score_threshold and generates an ROC curve.")
    ...
    if args.get_roc:
        ...
        score_threshes = [.01, .1, .2, .3, .4, .5, .6, .7, .8, .9, .99]
        metrics = []
        for score_thresh in score_threshes:
            args_copy = deepcopy(args)
            args_copy.score_threshold = score_thresh

            q = mp.Queue()
            p = mp.Process(target=_queue_evaluate, args=(args_copy, q))
            p.start()
