...
from .utils.eval_utils import calc_recall, calc_precision
        ...
        for epoch in range(args.total_epoches):
            ...
            for i in trange(args.train_batch_num):
                ...
                if __global_step % args.train_evaluation_step == 0 and __global_step > 0:
                    ...
                    precision = calc_precision(true_positive_dict, pred_labels_dict)
                    
                    info = "Epoch: {}, global_step: {} | loss: total: {:.2f}, xy: {:.2f}, wh: {:.2f}, conf: {:.2f}, class: {:.2f}, bboxes: {}/{} | ".format(
                            epoch, int(__global_step), loss_total.average, loss_xy.average, loss_wh.average, loss_conf.average, loss_class.average, int(n_pred_boxes), int(n_true_boxes))
...
if __name__ == '__main__':
    import argparse
    from .utils.config_utils import YoloArgs

    parser = argparse.ArgumentParser(description="YOLO-V3 train procedure.")
    parser.add_argument("--config_file", type=str, default="./data/my_data/config.yaml",
                    help="The path of the yaml configuration file.")
    args = YoloArgs(parser.parse_args().config_file)
